...
mod pbkdf2_aes;
use pbkdf2_aes::{decrypt, derive_key_from_password, encrypt, PasswordEntry};
...
impl MyApp {
    fn load_passwords(&mut self) -> Vec<PasswordEntry> {
        let mut stmt = self
            .conn
            .prepare("SELECT id, site, username, password FROM passwords")
            ...
            let decrypted_passwords: Vec<PasswordEntry> = self
                ...
                .map(|entry| {
                    let decrypted_password =
                        decrypt(&self.master_password, &entry.password).expect("Decryption failed");
                    PasswordEntry {
                        id: entry.id,
                        site: entry.site,
                                ...
                                if ui.button("Add Entry").clicked() {
                                    if let Some(_key) = self.aes_key {
                                        let encrypted_password =
                                            encrypt(&self.master_password, &self.password)
